...
def generate(n=N) -> pd.DataFrame:
    ...
    sugary_drinks_per_day = RNG.poisson(1.0, size=n)
    ...
    uses_fluoride_toothpaste = RNG.choice([0, 1], size=n, p=[0.2, 0.8])
    mouthwash_use = RNG.choice([0, 1], size=n, p=[0.6, 0.4])
    teeth_grinding = RNG.choice([0, 1], size=n, p=[0.8, 0.2])
    chews_sugarfree_gum = RNG.choice([0, 1], size=n, p=[0.75, 0.25])
    ...
    # smoking, access to care, age, protective habits like fluoride/gum).
    ...
    risk_score = (
        ...
        + 0.30 * sugary_drinks_per_day
        - 0.5 * brushing_freq_per_day
        - 0.4 * flosses
        + 0.5 * smokes
        - 0.3 * dental_visit_last_year
        - 0.3 * uses_fluoride_toothpaste
        - 0.2 * mouthwash_use
        - 0.15 * chews_sugarfree_gum
        + 0.15 * teeth_grinding
        + income_penalty
        + 0.02 * age
    ...
    df = pd.DataFrame({
        ...
        "uses_fluoride_toothpaste": uses_fluoride_toothpaste,
        "mouthwash_use": mouthwash_use,
        "teeth_grinding": teeth_grinding,
        "chews_sugarfree_gum": chews_sugarfree_gum,
