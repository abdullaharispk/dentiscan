...
def probability_to_tier(proba: float) -> str:
    return "High" if proba >= 0.6 else "Moderate" if proba >= 0.3 else "Low"
...
def predict_risk(pipeline, **kwargs) -> dict:
    ...
    return {"caries_probability": round(float(proba), 3), "risk_tier": probability_to_tier(proba)}
